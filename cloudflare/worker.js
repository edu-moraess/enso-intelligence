...
function seasonDate(season, year) {
  return `${year}-${String(SEASON_CENTRAL_MONTH[season]).padStart(2, "0")}-15`;
}
...
function parseRoni(text) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    const year = Number(yearRaw), value = Number(valueRaw);
    if (!Number.isInteger(year) || !Number.isFinite(value) || value <= -99) continue;
    rows.push({ date: seasonDate(season, year), season, year, roni: value });
  }
...
function parseOni(text) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (!SEASONS.has(season)) continue;
    const year = Number(yearRaw), total = Number(totalRaw), anomaly = Number(anomalyRaw);
    if (!Number.isInteger(year) || !Number.isFinite(total) || !Number.isFinite(anomaly) || anomaly <= -99) continue;
    rows.push({ date: seasonDate(season, year), season, year, total, oni: anomaly });
  }
...
function parseWeeklyNino(text) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    rows.push({ date, nino12_sst: nums[0], nino12: nums[1], nino3_sst: nums[2], nino3: nums[3], nino34_sst: nums[4], nino34: nums[5], nino4_sst: nums[6], nino4: nums[7] });
  }
...
function validateRows(rows, required) {
  if (!Array.isArray(rows) || !rows.length) throw new Error("Dataset contains no rows");
...
async function githubRequest(env, path, options = {}) {
  return fetch(`https://api.github.com/repos/${env.GITHUB_REPOSITORY}/contents/${path}${options.query || ""}`, {
...
async function publishDataset(env, name, rows, columns, sourceUrl) {
  ...
  if (!snapshotResponse.ok && snapshotResponse.status !== 422) throw new Error(`GitHub snapshot publish failed: ${snapshotResponse.status} ${await snapshotResponse.text()}`);

  let existingManifest = "", manifestSha;
  const manifestGet = await githubRequest(env, manifestPath, { query: `?ref=${encodeURIComponent(env.GIT_BRANCH)}` });
  if (manifestGet.ok) {
  ...
  if (!manifestResponse.ok) throw new Error(`GitHub manifest publish failed: ${manifestResponse.status} ${await manifestResponse.text()}`);
...
async function run(env) {
  ...
  return { foundation_version: FOUNDATION_VERSION, retrieved_at: new Date().toISOString(), results, errors };
}
...
export default {
  async scheduled(controller, env) {
    ...
    if (Object.keys(result.errors).length) throw new Error(`Foundation run failed: ${JSON.stringify(result.errors)}`);
  },
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname === "/health") return new Response(JSON.stringify({ status: "ok", service: "enso-data-core", foundation_version: FOUNDATION_VERSION }), { headers: { "Content-Type": "application/json" } });
    if (url.pathname === "/run") {
      const result = await run(env);
      return new Response(JSON.stringify(result, null, 2), { status: Object.keys(result.errors).length ? 500 : 200, headers: { "Content-Type": "application/json" } });
    }
    return new Response("ENSO Data Core", { status: 200, headers: { "Content-Type": "text/plain" } });
  },
};
