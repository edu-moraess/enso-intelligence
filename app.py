...
def window_start(x: pd.Series, years: int) -> pd.Timestamp: return max(x.iloc[0], x.iloc[-1] - pd.DateOffset(years=years))
def chart_layout(height: int, hovermode: str = "x", legend=None):
    d=dict(height=height,margin=dict(l=8,r=8,t=18,b=8),hovermode=hovermode,plot_bgcolor="#fff",paper_bgcolor="#fff",font=dict(color="#475569"),xaxis=dict(showgrid=False),yaxis=dict(zeroline=False,gridcolor="#edf2f7"))
    if legend is not None: d["legend"]=legend
    return d
roni_df,roni_meta=get_roni(); oni_df,oni_meta=get_oni(); nino_df,nino_meta=get_nino()
...
if roni_df is None or roni_df.empty:
    ...
    latest=roni_df.iloc[-1]; roni_val=float(latest["roni"]); state=classify_enso_state(roni_val); intensity=classify_intensity(roni_val); period=f"{latest['season']} {int(latest['year'])}"; trend_label,trend_delta=compute_recent_trend(roni_df["roni"],n_seasons=3); state_color={"El Niño":"#b91c1c","La Niña":"#1d4ed8","Neutral":"#15803d"}[state.value]
    ...
    x=pd.to_datetime(roni_df["date"]) if "date" in roni_df.columns else pd.to_datetime(roni_df.index); ymin=min(-2.2,float(roni_df["roni"].min())-.2); ymax=max(2.2,float(roni_df["roni"].max())+.2)
    fig=go.Figure(); fig.add_hrect(y0=.5,y1=ymax,fillcolor="rgba(220,38,38,.055)",line_width=0); fig.add_hrect(y0=-.5,y1=.5,fillcolor="rgba(22,163,74,.025)",line_width=0); fig.add_hrect(y0=ymin,y1=-.5,fillcolor="rgba(37,99,235,.055)",line_width=0); fig.add_hline(y=.5,line_color="#dc2626",line_dash="dash",line_width=1,annotation_text="El Niño +0.5°C",annotation_position="top left"); fig.add_hline(y=-.5,line_color="#2563eb",line_dash="dash",line_width=1,annotation_text="La Niña −0.5°C",annotation_position="bottom left"); fig.add_hline(y=0,line_color="#94a3b8",line_width=1)
    fig.add_trace(go.Scatter(x=x,y=roni_df["roni"],mode="lines",name="RONI",line=dict(color="#0f766e",width=2.5),hovertemplate="%{x|%b %Y}<br>RONI: %{y:+.2f} °C<extra></extra>")); fig.add_trace(go.Scatter(x=[x.iloc[-1]],y=[roni_val],mode="markers",name="Latest",marker=dict(size=10,color="#0f172a"),hovertemplate=f"{period}<br>RONI: {roni_val:+.2f} °C<extra></extra>"))
    lay=chart_layout(455); lay.update(yaxis=dict(title="RONI (°C)",range=[ymin,ymax],zeroline=False,gridcolor="#edf2f7"),xaxis=dict(range=[window_start(x,30),x.iloc[-1]],showgrid=False,rangeslider=dict(visible=True,thickness=.045),rangeselector=dict(buttons=[dict(count=10,label="10Y",step="year",stepmode="backward"),dict(count=30,label="30Y",step="year",stepmode="backward"),dict(step="all",label="All")])),showlegend=False); fig.update_layout(**lay); st.plotly_chart(fig,use_container_width=True,config={"displaylogo":False,"responsive":True})
...
if nino_df is None or nino_df.empty: data_unavailable_message(nino_meta.source,nino_meta.message)
else:
    regions=[("nino12_ssta","Niño 1+2"),("nino3_ssta","Niño 3"),("nino34_ssta","Niño 3.4"),("nino4_ssta","Niño 4")]; available=[r for r in regions if r[0] in nino_df.columns]
    if available:
        latest_week=pd.to_datetime(nino_df.iloc[-1]["date"]); st.markdown(f'<div class="chart-meta">Última observação semanal · {latest_week.strftime("%d %b %Y")}</div>',unsafe_allow_html=True); cards=st.columns(4)
        ...
        for col,label in available: fig.add_trace(go.Scatter(x=pd.to_datetime(nino_df["date"]),y=nino_df[col],mode="lines",name=label,line=dict(width=2.8 if label=="Niño 3.4" else 1.9,color=series_colors[label]),hovertemplate=f"%{{x|%d %b %Y}}<br>{label}: %{{y:+.2f}} °C<extra></extra>"))
        fig.add_hline(y=0,line_color="#94a3b8",line_width=1); nino_x=pd.to_datetime(nino_df["date"]); lay=chart_layout(350,"x unified",dict(orientation="h",y=1.07,x=0)); lay.update(yaxis=dict(title="Relative SST anomaly (°C)",zeroline=False,gridcolor="#edf2f7"),xaxis=dict(range=[window_start(nino_x,5),nino_x.iloc[-1]],showgrid=False,rangeslider=dict(visible=True,thickness=.045),rangeselector=dict(buttons=[dict(count=2,label="2Y",step="year",stepmode="backward"),dict(count=5,label="5Y",step="year",stepmode="backward"),dict(step="all",label="All")])))
...
st.markdown('<div class="section-rule"></div>',unsafe_allow_html=True)
section_header("Climate Context","O ENSO ajuda a explicar padrões climáticos de grande escala; os efeitos locais variam por região e estação.")
...
with st.expander("Methodological details"): st.markdown("**Anomaly = observed SST − climatological reference.** Os produtos semanais Niño usam os produtos de SST relativa do NOAA CPC. RONI e ONI são séries relacionadas, mas metodologicamente distintas; o RONI é o indicador operacional principal deste observatório e o ONI é complementar. Uma observação descreve condições atuais ou históricas; uma previsão estima condições futuras com incerteza.")
with st.expander("ENSO 101"): st.markdown("ENSO é um padrão acoplado oceano-atmosfera no Pacífico tropical. El Niño é a fase quente, La Niña a fase fria e Neutral descreve condições entre os limiares operacionais. Niño 3.4 é uma região de referência no Pacífico equatorial. As teleconexões variam conforme a localização e a estação.")
st.markdown('<div class="section-rule"></div>',unsafe_allow_html=True)
section_header("RONI vs ONI","Dois indicadores NOAA relacionados, mas com metodologias diferentes.")
if oni_df is not None and not oni_df.empty:
    oni_latest=oni_df.iloc[-1]; oni_val=float(oni_latest["oni"]); oc1,oc2,oc3=st.columns(3)
    with oc1: metric_card("RONI",f"{roni_val:+.2f} °C",f"primary · {period}")
    with oc2: metric_card("ONI",f"{oni_val:+.2f} °C",f"complementary · {oni_latest['season']} {int(oni_latest['year'])}")
    with oc3: metric_card("Difference",f"{roni_val-oni_val:+.2f} °C","RONI − ONI")
    rx=pd.to_datetime(roni_df["date"]) if "date" in roni_df.columns else pd.to_datetime(roni_df.index); ox=pd.to_datetime(oni_df["date"]) if "date" in oni_df.columns else pd.to_datetime(oni_df.index)
    ...
    if not comp.empty:
        cf=go.Figure(); cf.add_trace(go.Scatter(x=comp["date"],y=comp["RONI"],mode="lines",name="RONI",line=dict(color="#0f766e",width=2.5),hovertemplate="%{x|%b %Y}<br>RONI: %{y:+.2f} °C<extra></extra>")); cf.add_trace(go.Scatter(x=comp["date"],y=comp["ONI"],mode="lines",name="ONI",line=dict(color="#64748b",width=1.8),hovertemplate="%{x|%b %Y}<br>ONI: %{y:+.2f} °C<extra></extra>")); cf.add_hline(y=.5,line_color="#dc2626",line_dash="dash",line_width=1); cf.add_hline(y=-.5,line_color="#2563eb",line_dash="dash",line_width=1); cl=chart_layout(300,"x unified",dict(orientation="h",y=1.08,x=0)); cl.update(yaxis=dict(title="Index (°C)",zeroline=False,gridcolor="#edf2f7"),xaxis=dict(showgrid=False)); cf.update_layout(**cl); st.plotly_chart(cf,use_container_width=True,config={"displaylogo":False,"responsive":True})
...
st.markdown('<div class="section-rule"></div>',unsafe_allow_html=True)
section_header("Data & Sources","Produtos oficiais do NOAA Climate Prediction Center usados no observatório.")
for meta in [roni_meta,oni_meta,nino_meta]: st.markdown(f'<div class="source-row"><strong>{meta.dataset}</strong><br><small>NOAA Climate Prediction Center (CPC) · dados oficiais</small></div>',unsafe_allow_html=True)
