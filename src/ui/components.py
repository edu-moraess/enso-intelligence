...
def _configure_observatory_chart(fig):
    ...
    fig.update_layout(margin=dict(l=max(72, int(current.get("l") or 0)), r=max(28, int(current.get("r") or 0)), t=max(42, int(current.get("t") or 0)), b=max(34, int(current.get("b") or 0)), pad=max(4, int(current.get("pad") or 0))))
...
def apply_light_theme() -> None:
    """Apply the shared theme and route ModelLBs inside the same Streamlit app."""
    ...
    if st.query_params.get("view") == "model":
        from src.ui.model_view import render_model_lbs
        render_model_lbs()
        st.stop()
...
def metric_card(label: str, value: str, detail: Optional[str] = None) -> None:
    ...
    if label == "Estado atual": current_class += " state-card"
    elif label == "RONI": current_class += " roni-card"
    detail_html = f'<div class="metric-detail" style="color:#6b7280;font-size:.82rem;margin-top:.3rem;">{detail}</div>' if detail else ""
    st.markdown(f'<div class="obs-card{current_class}"><div class="section-label">{label}</div><div class="metric-value" style="font-size:1.55rem;font-weight:750;color:#111827;line-height:1.15;">{value}</div>{detail_html}</div>', unsafe_allow_html=True)


def render_footer() -> None:
    """Render the footer and switch to ModelLBs within the same app view."""
